...
<script lang="ts">
  ...
  class Particle {
    ...
    draw(context: CanvasRenderingContext2D) {
      context.fillStyle = this.color.replace('1)', `${this.life / this.maxLife})`);
      context.beginPath();
  ...
  // --- Animations ---
  const entryEffects = [
    () => gsap.from(".auth-container", { y: -100, opacity: 0, duration: 1.2, ease: "back.out(1.7)" }),
    () => gsap.from(".auth-container", { scale: 0.7, opacity: 0, duration: 1, ease: "expo.out" }),
    () => gsap.from(".auth-container", { rotationX: 90, opacity: 0, duration: 1, transformOrigin: "top center", ease: "power2.out" }),
    () => gsap.from(".auth-container", { y: 60, opacity: 0, duration: 1, ease: "power3.out" }),
  ];
  ...
  function animateFlip(callback: () => void) {
    ...
    gsap.to(container, {
      ...
      onStart: () => {
        ...
        if (shimmer) gsap.fromTo(shimmer, { opacity: 0, scale: 0.8 }, { opacity: 1, scale: 1.2, duration: 0.3 });
      },
      onComplete: () => {
        callback();
        gsap.fromTo(
