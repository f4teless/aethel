...
<div class={`w-full z-20 transition-colors duration-300
  ${transparent && !mobileMenuOpen ? 'sticky top-0 left-0' : 'bg-transparent backdrop-blur-lg sticky top-0 shadow-xl'}
`}>
  <header class="px-6 md:px-12 py-4 flex justify-between items-center font-cinzel uppercase tracking-wider text-white text-sm">
    ...
    <nav class="space-x-6 hidden md:block text-slate-300 font-medium">
      ...
      <a href="/community" class="hover:text-white transition" class:text-white={page.url.pathname === '/community'}>Community</a>
    ...
    <div class="flex items-center space-x-4">
      ...
      <div class="hidden md:block">
        ...
        {:else}
          <a 
            href="/login" 
            class="px-4.5 py-2 font-ebg shadow-md bg-transparent text-white rounded-md transition  hover:bg-current/20 hover:shadow-xl duration-200 border border-white/20 backdrop-blur-sm"
          >
            Log In
          </a>
        {/if}
  ...
  {#if mobileMenuOpen}
    ...
    >
      ...
      <a href="/community" class="text-slate-300 hover:text-white text-xl">Community</a>
